...
__all__ = ['HourglassNet_ConvLSTM', 'hg_convlstm']
...
class HourglassNet_ConvLSTM(nn.Module):
    '''Hourglass model from Newell et al ECCV 2016'''
    def __init__(self, block, num_stacks=2, num_blocks=4, num_classes=16):
        super(HourglassNet_ConvLSTM, self).__init__()
        ...
        # 2020.6.14
        # horglass_convLSTM setting
        num_stacks_ConvLSTM = 2
        for i in range(num_stacks_ConvLSTM):
            hg.append(Hourglass(block, num_blocks, self.num_feats, 4))
            res.append(self._make_residual(block, self.num_feats, num_blocks))
            fc.append(self._make_fc(ch, ch))
            score.append(nn.Conv2d(ch, num_classes, kernel_size=1, bias=True))
            if i < num_stacks_ConvLSTM-1:
...
def hg_convlstm(**kwargs):
    model = HourglassNet_ConvLSTM(Bottleneck, num_stacks=kwargs['num_stacks'], num_blocks=kwargs['num_blocks'],
